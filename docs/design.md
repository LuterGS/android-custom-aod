...
1분 단위로 7×7 물리 픽셀 범위에서 위치를 이동한다. 표시는 사용자 배율을 그대로 적용하며 화면 크기에 맞춰 자동 축소하지 않는다. Surface가 바뀌면 1Hz 힌트를 다시 설정한다.
...
나머지 개수는 아래에 요약한다. 긴 글은 한 줄 말줄임으로 처리하고, 화면 아래로 넘치는 콘텐츠는 잘린다.
...
시스템 알림의 `postTime`을 사용해 각 행 오른쪽에 게시 시각을 표시한다. 기본은 상대시각이며
1분 미만은 ‘방금’, 이후는 경과한 정수 분을 표시한다. 절대시각은 현재 시간대의 `HH:mm`이다.
앱별로 묶인 행은 가장 최근 항목의 시각을 사용하고 개수와 함께 표시한다. 시간 영역의 너비를
앱 이름에서 예약하여 긴 이름과 겹치지 않게 한다. 잘못된 시각은 ‘—’, 미래 시각의 상대 표시는
‘방금’으로 처리한다. 기존 분 단위 Surface 갱신만 사용하며 새 타이머를 추가하지 않는다.

### 크기·테마 설정

시계·날짜·오전/오후 크기와 전체 배율을 독립적으로 50~200%로 저장한다. 날짜·오전/오후 기본 크기도
15→17, 10→14 디자인 단위로 확대했다. 시계 크기에 따라 열 너비와 줄 간격을 계산하며,
화면 높이·너비와 관계없이 설정 배율을 유지하며 넘치는 부분은 Surface 경계에서 잘린다.
상단은 카메라 컷아웃 안전 영역 아래 24dp의 고정 여백을 둔다. 오전/오후와 배터리·충전 예상 시간을
같은 줄에 배치하고 24시간제에서는 배터리만 가운데 표시한다. 물리 픽셀 번인 이동은 여전히 확대 전에 적용한다.
